...
int inet_listen(struct socket *sock, int backlog)
{
	...
	if (old_state != TCP_LISTEN) {
		...
		// 监听操作的核心逻辑，5.10版本中没有申请半连接队列的空间，而3.10里计算了半连接队列长度并申请了空间
...
lookup_protocol:
	...
	// RCU（Read-Copy Update）是一种用于实现高效读取和并发更新数据结构的同步机制。在网络内核中，它允许多个线程或进程同时读取共享数据，而不需要获取锁，从而提高了并发性能。
...
{
	// 设置 sk中的sk_state，实际为 __sk_common.skc_state
