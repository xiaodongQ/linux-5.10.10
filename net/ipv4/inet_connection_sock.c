...
int inet_csk_listen_start(struct sock *sk, int backlog)
{
	...
	// ~~废弃：icsk_accept_queue 是全连接队列，已经`ESTABLISHED`的队列（FIFO of established children）~~
	// icsk_accept_queue里面包含全连接队列、以及半连接队列长度等信息
	// 此处仅初始化了一些成员变量，不同版本内核实现有差异（参考连接里是3.10，里面做了内存申请，实现差异较大）
	// 注意下面这个说法是对于3.10内核来说的：我们平时说的全连接队列、半连接队列全部都是在这个数据结构里实现的。
