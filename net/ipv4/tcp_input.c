		...
		default:
			...
			case TCPOPT_TIMESTAMP:
				if ((opsize == TCPOLEN_TIMESTAMP) &&
				    ((estab && opt_rx->tstamp_ok) ||  // tstamp_ok为1，表示时间戳选项是否已协商通过
				     (!estab && net->ipv4.sysctl_tcp_timestamps))) { // 本地开启了sysctl_tcp_timestamps内核参数
					// 协商过时间戳选项或者本地开启了tcp_timestamps，则对时间戳进行记录
					opt_rx->saw_tstamp = 1;
...
static bool tcp_syn_flood_action(const struct sock *sk, const char *proto)
{
	...
	// net.ipv4.tcp_syncookies 内核参数，一般是开的
		     ...
		     struct sock *sk, struct sk_buff *skb)
{
	...
	if ((net->ipv4.sysctl_tcp_syncookies == 2 ||
		...
		// 主要检查是否开启了 net.ipv4.tcp_syncookies，SYN flood攻击
	...
	*/ 
	...
	// 初始化 inet_request_sock 和 tcp_request_sock 相关的一些字段
