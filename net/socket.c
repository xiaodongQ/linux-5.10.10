			 ...
			 struct socket **res, int kern)
{
	 ...
	 */
	// 从全局里根据协议族找对应协议 `struct net_proto_family __rcu *net_families[NPROTO]`
	// 全局中是根据inet_init网络初始化时通过`sock_register`进行协议添加的，只加了一种：struct net_proto_family inet_family_ops
	...
	rcu_read_lock();
	// 根据协议族找到协议
	// rcu（Read-Copy Update）是一种实现高效读取和并发更新数据结构的同步机制
	...
	// struct net_proto_family inet_family_ops 协议族，注册的`create`接口为`inet_create`
...
int __sys_socket(int family, int type, int protocol)
{
	...
	// 创建一个 `struct socket` 实例
	retval = sock_create(family, type, protocol, &sock);
	if (retval < 0)
		return retval;

	// 绑定 `struct socket`到一个文件，并返回fd
